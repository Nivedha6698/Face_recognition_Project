...
import cv2, face_recognition, numpy as np,pyodbc
from models.db_engine import mydb,cursor
        ...
        while True:
            ...
            if cv2.waitKey(1) & 0xFF == ord('q'):
                ...
                if (img_filename is not None) and (len(face_encodings) > 0):
                    ...
                    cursor.execute("INSERT INTO images (name, encoding, image) VALUES (?, ?, ?);",
                                   (img_filename, val, img_data))
                    cursor.execute("UPDATE images SET authenticate = 'yes' WHERE image = ?", (img_data,))
                else:
